...
function Update()
	...
	-- READ FILE
	-- io.open, not io.input: io.input raises an error when the file is missing,
	-- and that error repeating on every update crashed Rainmeter outright. A
	-- missing note (Notes.txt is gitignored, so every fresh clone) is created
	-- empty instead, which also gives the click-to-edit action a file to open.
	local File    = io.open(Notes[n].Path, 'r')
	if not File then
		local New = io.open(Notes[n].Path, 'a')
		if New then
			New:close()
			File = io.open(Notes[n].Path, 'r')
		end
	end
	local Content = nil
	if File then
